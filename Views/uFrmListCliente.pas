...
var
  FrmInforCliente: TFrmInforCliente;
...
procedure TFrmInforCliente.BuscarCliente;
begin
...
FDQuery1.Open;
 ...
 begin
    ...
    EdtNome.SelectAll;
    EdtNome.SetFocus;
    FrmMsgerro.MsgText('Cliente não encontrado');
   end;
 end;
end;

procedure TFrmInforCliente.EdtNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_ESCAPE then
    ...
    begin//Se  estiver com o foco fecha a busca de cliente
      ...
      FrmInforCliente.close;
    end;
end;

procedure TFrmInforCliente.EdtNomeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFrmInforCliente.FormActivate(Sender: TObject);
begin
ArredondaPainel(pnlFundo,12);
ArredondaPainel(pnlListFundo,12);
end;

procedure TFrmInforCliente.FormClose(Sender: TObject; var Action: TCloseAction);
begin
FrmEsmaecerTela.Hide;
Action := caFree;
end;

procedure TFrmInforCliente.FormCreate(Sender: TObject);
begin

FrmEsmaecerTela.Show;
end;

procedure TFrmInforCliente.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if acao  = true then
  EdtNome.SetFocus; // QUando apertar qualquer tecla foca no edit

end;

procedure TFrmInforCliente.FormResize(Sender: TObject);
begin

  pnlfundo.Top  := Round(FrmInforCliente.Height/2 - pnlFundo.Height/2);
  pnlfundo.Left := Round(FrmInforCliente.Width/2 - pnlFundo.Width/2);
end;

procedure TFrmInforCliente.FormShow(Sender: TObject);
begin
...
procedure TFrmInforCliente.gridClientesDrawColumnCell(Sender: TObject;
...
procedure TFrmInforCliente.gridClientesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
